...
pub async fn auth(
    ...
    Ok(aurora::service::auth(&state.conn, payload.login, payload.password).await?)
}
...
pub async fn has_joined(
    ...
    Ok(aurora::service::has_join(&state.conn, payload).await?)
}
...
pub async fn profile(
    ...
    Ok(aurora::service::profile(&state.conn, payload).await?)
}
