...
- **Поддержка S3 временно отключёна**. S3 Нужен для хранения текстур скинов и плащей игроков. Используйте Amazon или подобные ему хранилища (Minio как из вариантов)
...
Пример комманды для генерации секрета для JWT токенов и Cookie: 
```
...
- JWT_SECRET - Секретный ключ для JWT. **Должен быть длиной 64 байта!**
- JWT_EXPIRES_IN - Время жизни JWT токена
- COOKIE_SECURE - Использовать HTTPS для куки
- COOKIES_SECRET - Секретный ключ для куки. **Должен быть длиной 64 байта!**
...
- MAX_CONNECTION="30" - количество соединений с базой данных
- CONNECT_TIMEOUT="10" - время ожидания покдлючения к базе данных
